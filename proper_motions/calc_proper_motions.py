...
rem_list = [np.where(all_ind==5)[0][0], np.where(all_ind==10)[0][0], np.where(all_ind==20)[0][0], np.where(all_ind==54)[0][0]] #src n, src I, BN, all of which are known to have high PM
...
ra_pm_err = np.std(ra_pm[ref_all_ind]) #question: only use reference source in std dev or all sources?
...
src_names[rem_list] = ['n', 'I', 'BN', '54']

for i in range(len(src_names)):
	plt.text(arrow_x_pix[i]+10, arrow_y_pix[i]+10, src_names[i], color='g')
