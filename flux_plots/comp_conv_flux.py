...
data_conv = Table.read('/users/jotter/summer_research_2018/tables/r0.5_catalog_conv_bgfitted_apflux_final.fits')
...
deconv_ind_conv = np.where(np.isnan(data_conv['ap_flux_'+band]) == False)[0]
deconv_ind_unconv = np.where(np.isnan(data['ap_flux_'+band]) == False)[0]
...
int_flux = (2*np.pi*data['gauss_amp_'+band]*data['fwhm_maj_'+band]*u.arcsec*data['fwhm_min_'+band]*u.arcsec*FWHM_TO_SIGMA**2/beam.sr).decompose()
int_flux_conv = (2*np.pi*data_conv['gauss_amp_'+band]*data_conv['fwhm_maj_'+band]*u.arcsec*data_conv['fwhm_min_'+band]*u.arcsec*FWHM_TO_SIGMA**2/beam_conv.sr).decompose()
